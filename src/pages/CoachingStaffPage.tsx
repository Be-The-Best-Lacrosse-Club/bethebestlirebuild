    ...
    <div className="min-h-screen bg-black text-white" style={{ fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <section className="relative pt-24 pb-20 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-[900px] mx-auto pt-16">
          <div className="text-lg font-bold uppercase tracking-[4px] text-[var(--btb-red)] mb-6">{label} Coaching Staff</div>
      ...
      <section className="py-24 px-6 bg-neutral-950 border-y border-white/[0.07]">
        <div className="max-w-[900px] mx-auto">
          <div className="text-lg font-bold uppercase tracking-[4px] text-[var(--btb-red)] mb-4">Staff</div>
            ...
            {data.coaches.map((coach) => (
              <div key={coach.name + coach.title} className="group rounded-2xl border border-white/[0.07] bg-white/[0.02] p-8 hover:border-white/[0.15] transition-all duration-300">
                ...
                <div className="flex items-start gap-5 mb-6">
                  ...
                  <div>
                    <div className="font-display text-xl uppercase tracking-wide text-white">{coach.name}</div>
                    <div className="text-lg font-bold uppercase tracking-[1.5px] text-[var(--btb-red)] mt-1">{coach.title}</div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {coach.credentials.map((cred) => (
                    <span key={cred} className="text-base font-bold uppercase tracking-[1px] px-3 py-1.5 rounded-full border border-white/[0.08] bg-white/[0.03] text-white/35">
                      {cred}
                    </span>
                  ))}
                </div>
                ...
                {/* Bio */}
                <p className="text-lg text-white/35 leading-relaxed group-hover:text-white/45 transition-colors">{coach.bio}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-6 border-b border-white/[0.07]">
        <div className="max-w-[900px] mx-auto">
          <div className="text-lg font-bold uppercase tracking-[4px] text-[var(--btb-red)] mb-4">Philosophy</div>
          <h2 className="font-display text-[clamp(2rem,4vw,3rem)] uppercase tracking-wide leading-[0.92] mb-5">
            How Our<br />Coaches Operate
          </h2>
          <p className="text-lg text-white/35 max-w-[460px] leading-relaxed mb-14">
          ...
          <div className="space-y-0 border-t border-white/[0.07]">
            {philosophy.map((item, i) => (
              <div key={i} className="flex items-start gap-8 py-8 border-b border-white/[0.07] group">
                ...
                <div>
                  <h4 className="font-display text-lg uppercase tracking-wide text-white group-hover:text-[var(--btb-red)] transition-colors mb-2">{item.title}</h4>
                  <p className="text-lg text-white/35 leading-relaxed">{item.text}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 px-6 bg-neutral-950 border-b border-white/[0.07]">
        <div className="max-w-[900px] mx-auto">
          <div className="text-lg font-bold uppercase tracking-[4px] text-[var(--btb-red)] mb-4">Requirements</div>
          <h2 className="font-display text-[clamp(2rem,4vw,3rem)] uppercase tracking-wide leading-[0.92] mb-14">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {certRequirements.map((req) => (
                  <div key={req} className="flex items-center gap-3 p-4 rounded-lg border border-white/[0.06] bg-white/[0.02]">
                    ...
                    <span className="text-base text-white/45">{req}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-[900px] mx-auto text-center">
          <div className="relative border border-[var(--btb-red)]/25 rounded-2xl px-10 py-14 overflow-hidden">
            ...
            <div className="relative">
              <div className="text-lg font-bold uppercase tracking-[4px] text-[var(--btb-red)] mb-6">{label} Program</div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <a href="mailto:info@bethebestli.com" className="inline-flex items-center justify-center gap-2 px-9 py-4 border border-white/15 text-white/78 text-base font-bold uppercase tracking-[2px] rounded hover:border-white/30 hover:text-white transition-all duration-200">
                  Email Us
                </a>
              </div>
            </div>
