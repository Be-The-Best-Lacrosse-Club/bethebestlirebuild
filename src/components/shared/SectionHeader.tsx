  ...
  return (
    <div ref={ref} className="mb-16 max-w-[680px]">
      <div className="flex items-center gap-3 mb-5">
        <span className={`text-lg font-bold tracking-[3px] uppercase ${light ? "text-black/30" : "text-white/25"}`}>
          {number}
        </span>
        <div className={`h-px w-8 ${light ? "bg-black/15" : "bg-white/10"}`} />
        <span className={`text-lg font-bold tracking-[3px] uppercase ${light ? "text-[var(--btb-red)]" : "text-[var(--btb-red)]"}`}>
          {label}
        </span>
      </div>
      <h2
      ...
      {subtitle && (
        <p className={`mt-4 text-xl leading-[1.8] ${light ? "text-black/50" : "text-white/70"}`}>
          {subtitle}
        </p>
      )}
    </div>
